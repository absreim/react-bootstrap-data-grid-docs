...
import {controlledFilterModel, uncontrolledFilterModel} from "../assets/filtering/typeSpecs";
import ControlledFilteredGrid from "../assets/filtering/ControlledFilteredGrid";
import {
    abstractDateFilterState, abstractFilterState,
    betweenDatesFilterState,
    endDateFilterState, numberFilterState,
    startDateFilterState, stringFilterState
...
By default, for the sake of backwards compatibility, the filtering interface can be accessed by clicking a toggle button
above the grid.
...
_Controlled_ mode allows the parent component full control over the state of filtering in the grid, but requires more
code to set up.

_Uncontrolled_ mode causes the `Grid` component itself to keep track of the state of filtering and this state is not
...
guidance as to how one can filter on the original data. For example, you may have a formatter on a number column to
