...
to enable [pagination](/docs/table/pagination). It has been renamed to `PaginationModel`. If any of your existing code is
...
Previously, there were two ways to access the [filtering](/docs/table/filtering) interface:

- A button labeled *Show Filter Options* that is by default located above the main table.
- A button on the [toolbar](/docs/table/toolbar).
...
These features are [filtering](/docs/table/filtering) and [export](/docs/table/export). Filtering is enabled by passing a value for
...
For both the [filtering](/docs/table/filtering) and [export](/docs/table/export) UIs, the *Submit* button now closes the interface.
...
To disable responsive behavior for a table, it is necessary to use the [styling customization](/docs/table/styling) feature
...
to the [style model](/docs/table/styling#stylemodel) for that element. These classes were:
...
[style model](/docs/table/styling##stylemodel) property.
...
All changes to the [Style Model](/docs/table/styling#stylemodel) occur in the `additionalComponentsStyleModel` property.

### Removed Properties

The following properties have been removed. Both of the below properties belong to the original way of accessing the
[filter options table](/docs/table/filtering). Since the [toolbar](/docs/table/toolbar) is now the only way of accessing
the filter
