...
This type contains the props for `GridPro`. The props are largely the same as `GridProps` but with a few differences.
...
additionally contains `reorderModel` property for customizing style of the [row reordering](/docs/reorder) feature.
- `GridProProps` contains a `reorderModel` prop to enable [row reordering](/docs/reorder) while `GridProps` does not.
